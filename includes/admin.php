...
class Admin {
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'elementor-admin-app', ELEMENTOR_ASSETS_URL . 'js/admin' . $suffix . '.js', [
				'jquery',
			], ELEMENTOR_VERSION, true );

		wp_localize_script( 'elementor-admin-app', 'ElementorAdminConfig', [
				'home_url' => home_url(),
				'i18n' => [
	...
	public function enqueue_styles() {
		...
		wp_register_style( 'elementor-admin-app', ELEMENTOR_ASSETS_URL . 'css/admin' . $direction_suffix . $suffix . '.css', [
				'elementor-icons',
			], ELEMENTOR_VERSION );
	...
	public function print_switch_mode_button( $post ) {
        ...
        <div id="elementor-switch-mode">
            <input id="elementor-switch-mode-input" type="hidden" name="_elementor_post_mode"
                   value="<?php echo Plugin::$instance->db->is_built_with_elementor( $post->ID ); ?>"/>
        ...
        <div class="notice updated is-dismissible elementor-message elementor-message-dismissed"
            ...
            <div class="elementor-message-inner">
                ...
                <div class="elementor-message-content">
                    ...
                    <p>
						<?php
						printf( /* translators: 1: details URL, 2: accessibility text, 3: version number, 4: update URL, 5: accessibility text */
							__( 'There is a new version of Elementor Page Builder available. <a href="%1$s" class="thickbox open-plugin-details-modal" aria-label="%2$s">View version %3$s details</a> or <a href="%4$s" class="update-link" aria-label="%5$s">update now</a>.', 'elementor' ), esc_url( $details_url ), esc_attr( sprintf( /* translators: %s: version number */
									__( 'View Elementor version %s details', 'elementor' ), $product->new_version ) ), $product->new_version, esc_url( $upgrade_url ), esc_attr( __( 'Update Now', 'elementor' ) ) );
						?>
                    </p>
                </div>
                <div class="elementor-message-action">
                    <a class="button elementor-button" href="<?php echo $upgrade_url; ?>"><i
                                class="dashicons dashicons-update"></i><?php _e( 'Update Now', 'elementor' ); ?></a>
                </div>
            </div>
        </div>
		<?php
	...
	public function admin_footer_text( $footer_text ) {
		...
		if ( $is_elementor_screen ) {
			$footer_text = sprintf( /* translators: %s: link to plugin review */
				__( 'Enjoyed <strong>Elementor</strong>? Please leave us a %s rating. We really appreciate your support!', 'elementor' ), '<a href="https://wordpress.org/support/plugin/elementor/reviews/?filter=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>' );
		}
	...
	public function enqueue_feedback_dialog_scripts() {
		...
		wp_register_script( 'elementor-admin-feedback', ELEMENTOR_ASSETS_URL . 'js/admin-feedback' . $suffix . '.js', [
				'jquery',
				'underscore',
				'elementor-dialog',
			], ELEMENTOR_VERSION, true );
		...
		wp_localize_script( 'elementor-admin-feedback', 'ElementorAdminFeedbackArgs', [
					...
					<?php foreach ( $deactivate_reasons as $reason_key => $reason ) : ?>
                        <div class="elementor-deactivate-feedback-dialog-input-wrapper">
                            ...
                            <label for="elementor-deactivate-feedback-<?php echo esc_attr( $reason_key ); ?>"
                                   class="elementor-deactivate-feedback-dialog-label"><?php echo $reason['title']; ?></label>
